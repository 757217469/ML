...
# 当penalty为l2的时候,参数可以使: lbfgs(拟牛顿法),newton-cg(牛顿法变种),seg(minibatch)
...
# class_weight: 特征权重参数
# Logistic回归是异种分类算法, 不能应用于回归中(也即是说对于传入模型的y值来讲, 不能使float类型,必须是int类型)
...
print('R值(准确率): ', r)
print('系数花特征比率: %.2f%%' % (np.mean(lr.coef_.ravel() == 0) * 100))
print('参数', re.coef_)
