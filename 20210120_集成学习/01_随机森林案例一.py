...
print('训练样本数量:%d,特征属性数目:%d,目标属性数目:%d' % (x_train.shape[0], x_train.shape[1], y_train.shape[0]))
...
forest_fpr2, forest_tpr2, _ = metrics.roc_curve(label_binarize(y_test[names[-3]], classes=(0, 1, 2)).T[:-1].T.ravel(),
                                                forest_y_score[1].ravel())
forest_fpr3, forest_tpr3, _ = metrics.roc_curve(label_binarize(y_test[names[-2]], classes=(0, 1, 2)).T[:-1].T.ravel(),
                                                forest_y_score[2].ravel())
forest_fpr4, forest_tpr4, _ = metrics.roc_curve(label_binarize(y_test[names[-1]], classes=(0, 1, 2)).T[:-1].T.ravel(),
...
label_binarize(y_test[names[-4]], classes=(0, 1, 2)).T[:-1].T.ravel()

label_binarize(['a', 'a', 'b', 'b'], classes=('a', 'b', 'c'))
label_binarize(['a', 'a', 'b', 'b'], classes=('a', 'b', 'c')).T[:-1].T.ravel()

# 正确的数据
y_true = label_binarize(y_test[names[-4]], classes=(0, 1, 2)).T[:-1].T.ravel()
...
plt.grid(b=True, ls=':')
plt.legend(loc='lower right', facybox=True, framealpha=0.8, fontsize=12)
...
plt.legend(loc='upper left', fancybox=True, framealpha=.8, fontsize=12)
plt.grid(True)
plt.xlim(min(depth), max(depth))
plt.ylim(min_err * .99, max_err * 1.01)
