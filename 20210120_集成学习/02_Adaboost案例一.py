...
# base_estimator: DecisionTreeClassifier 选择弱分类器, 默认CART树
# algorithm: SAMME 和 SAMME.R . 运算规则, 后者是优化算法, 以概率调整权重, 迭代速度快
# 需要能计算概率的分类器支持
# learning_rate: 0<v<=1,默认为1, 正则项衰减系数
...
plot_colors = 'br'
class_names = 'AB'
plt.figure(figsize=(10, 5), facecolor='2')
# 局部子图
plt.subplot(121)
...
plt.xlabel('x')
plt.ylabel('y')
plt.title(u'AdaBoost分类结果, 正确率为:%2.f%%' % (bdt.score(X, y) * 100))
