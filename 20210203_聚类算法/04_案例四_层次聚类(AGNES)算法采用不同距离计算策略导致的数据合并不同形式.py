...
n_cluster = 4
N = 1000
data1, y1 = ds.make_blobs(n_samples=N, n_features=2, centers=((-1, 1), (1, 1), (1, -1)), random_state=0)
...
for index, (n_clusters, data, y) in enumerate(
        ((4, data1, y1), (4, data1_noise, y1_noise), (2, data2, y2), (2, data2_noise, y2_noise))):
    ...
    # 计算类别与类别的聚类(只计算最接近的7个样本的聚类) -- 希望在AGNES算法中,在计算过程中不需要重复性的计算点与点之间的距离
...
plt.tight_layout(.5, rect=(0, 0, 1, .95))
plt.show()
