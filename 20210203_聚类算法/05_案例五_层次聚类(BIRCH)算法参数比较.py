...
X, y = make_blobs(n_samples=100000, n_features=2, centers=n_centers, random_state=28)

# 创建不同的参数(簇直径) Birch层次聚类
birch_models = [
    Birch(threshold=1.7, n_clusters=None),
    Birch(threshold=.5, n_clusters=None),
    Birch(threshold=1.7, n_clusters=100)
]
# threshold: 簇直径的阈值,   branching_factor: 大叶子个数

# 我们也可以加参数来试一下效果, 比如加入分支因子 branching_factor, 给定不同的参数值, 看聚类的结果

final_step = [u'直径=1.7,n_clusters=None', u'直径=0.5,n_clusters=None', u'直径=1.7,n_clusters=100']
plt.figure(figsize=(12, 8), facecolor='w')
plt.subplots_adjust(left=.02, right=.98, bottom=.1, top=.9)
# cycle 无穷循环器
...
for ind, (birch_model, info) in enumerate(zip(birch_models, final_step)):
    ...
    print('Birch算法, 参数信息为: %sd; 模型构建消耗时间为:%.3fs; 聚类中心数目:%d' % (info, time_, len(np.unique(labels))))
