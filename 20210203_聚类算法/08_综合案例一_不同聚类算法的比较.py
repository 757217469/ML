...
# 高斯分布
blobs = ds.make_blobs(n_samples=n_samples, n_features=2, cluster_std=.5, centers=3, random_state=0)
no_structure = np.random.rand(n_samples, 2), None

datasets = [noisy_circles, noisy_moons, blobs, no_structure]
clusters = [2, 2, 3, 2]
clustering_names = ['KMeans', 'MiniBatchMeans', 'AC-ward', 'AD-average', 'Birch', 'DBSCAN', 'SpectralClustering']
plt.figure(figsize=(len(clustering_names) * 2 + 3, 9.5), facecolor='w')
plt.subplots_adjust(left=.2, right=.98, bottom=.001, top=.96, wspace=.05, hspace=.01)
...
for i_dataset, (dataset, n_cluster) in enumerate(zip(datasets, clusters)):
    ...
    for name, algorithm in zip(clustering_names, clustering_algorithms):
        ...
        algorithm.fit(X)
        t1 = time.time()
        # 如果模型中存在"labels_'这个属性的话, 那么获取这个预测的类别值
