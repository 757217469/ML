...
y = pd.Categorical(y).codes  # 把文本数据进行编码, 比如a b c编码为0 1 2;可以通过pd.Categorical(y).categories获取index对应的原始值
x = x[[0, 1]]  # 获取第一列和第二列
...
kernal: 置顶SVM内部核函数的类型, 可选值: linear, poly, rbf,sigmoid,precomputed(基本不用, 有前提要求, 要求特征属性数目和样本数目一样);默认是rbf;
degree: 当使用多项式函数作为svm内部的核函数的时候, 给定多项式的项数, 默认为3
gamma: 当SVM内部使用poly,rbf,sigmoid的时候,核函数的系数值,当默认值为auto的时候,实际系数为1/n_features
coef0: 当核函数为poly或者sigmoid的时候, 给定的独立系数,默认为0
probability:是否启用概率估计,默认不启用,不太建议启用
shrinking: 是否开启收缩启发式计算,默认为True
tol: 模型构建收敛参数, 当模型的误差变化率小于该值的时候,结束模型构建过程,默认1e-3
cache_size: 在模型构建过程中, 缓存数据的最大内存大小,默认为空,单位MB
class_weight: 给定各个类别的权重,默认为空
max_iter: 最大迭代次数,默认-1表示不限制
decision_function_shape: 决策函数, 可选值: ovo和ovr,默认为None;推荐使用ovr; (1.7版本以上才有)
'''
# 数据SVM分类器构建
clf = svm.SVC(C=1, kernel='rbf', gamma=0.1)
# gamma值越大, 训练集的你和就越好, 但是会造成过拟合, 导致测试集你和变差
