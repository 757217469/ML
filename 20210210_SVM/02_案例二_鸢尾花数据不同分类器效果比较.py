...
rc = RidgeClassifier()  # ridge是为了解决特征大于样本, 反而导致分类效果较差的情况,而提出的
...
# 所有样本点
plt.scatter(x[0], x[1], c=y, edgecolors='k', s=50, cmap=cm_dark,zorder=10)
# 测试数据集
plt.scatter(x_test[0], x_test[1],c=svm.predict(x_test), s=120, facecolor='none', zorder=1)
# label列表
...
plt.xlim(x1_min, x1_max)
plt.ylim(x2_min, x2_max)
plt.title(u'鸢尾花数据Ridge特征分类', fontsize=16)
plt.grid(b=True, ls=':')
plt.tight_layout(pad=1.5)
