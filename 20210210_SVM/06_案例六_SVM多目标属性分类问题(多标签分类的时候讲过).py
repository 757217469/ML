...
def plot_subfigure(X, Y, subplot, title, transform):
    ...
    # 多分类使用OneVsRestClaaifier进行多个目标的分类问题的专门算法API
    ...
    # 画出类别1的数据
    plt.scatter(X[zero_class, 0], X[zero_class, 1], s=160, edgecolors='b', facecolors='none', linewidth=2, label=u'类别1')
    # 画出类别2的数据
...
X, Y = make_multilabel_classification(n_samples=100, n_features=30, n_classes=2, n_labels=2,
...
# 图表展示
plt.subplots_adjust(.04, .02, .97, .09, .2)
plt.show()
