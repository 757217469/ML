...
def benchmark(clf, name):
    ...
    print(u'5折交叉验证的训练时间为: %.3fs/(5*%d)=%.3fs' % ((t_end - t_start), m, t_train))
...
print('%d文本数量 - %.3fMB(训练数据集)' % (len(data_train.data), data_train_size_mb))
print('%d文本数量 - %.3fMB(测试数据集)' % (len(data_test.data), data_test_size_mb))
...
vectorizer = TfidfVectorizer(input='content', stop_words='english', max_df=.5, sublinear_tf=True)
x_train = vectorizer.fit_transform(data_train.data)  # x_train是稀疏的, scipy.sparse.csr.csr_matrix
x_test = vectorizer.transform(data_test.data)
print(u'训练集个数: %d, 特征个数: %d' % x_train.shape)
print(u'停止词:\n')
...
clfs = [
    ...
    [LinearSVC(loss='squared_hinge', penalty='l2', dual=False, tol=1e-4), 'LienarSVC-l2']
]
# 将训练数据保存到一个列表中
result = []
...
ax = plt.axes()
b0 = ax.bar(x + .1, training_err, width=.2, color='#77E0a0')
b1 = ax.bar(x + .3, test_err, width=.2, color='#8800FF')
ax2 = ax.twinx()
b2 = ax2.bar(x + .5, training_time, width=.2, color='#FFA0A0')
b3 = ax2.bar(x + .7, test_time, width=.2, color='#FF8080')
plt.xticks(x + .5, clf_names)
