...
print('数据样本数据量:%d, 特征数量:%d' % data.shape)
data_x = data[data.columns[1:]]
data_y = data[data.columns[0]]
data.head()

x, x_test, y, y_test = train_test_split(data_x, data_y, train_size=.6, random_state=0)
...
x1_d = (x1_max - x1_min) * .05
x1_min -= x1_d
x1_max += x1_d
x2_d = (x2_max - x2_min) * .05
x2_min -= x2_d
x2_max += x2_d
