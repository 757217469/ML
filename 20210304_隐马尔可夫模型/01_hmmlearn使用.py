...
'''
hmmlearn框架说明
hmmlearn中主要有两种模型, 分布为: GaussianHMM和MultinomialHMM; 如果观测值是连续的,
那么建议使用GaussianHMM,否则使用MultinomialHMM; 参数: 初始的隐藏状态概率π参数为:
startprob_; 状态转移矩阵A参数为: transmat_; 状态和观测值之间的转移矩阵B参数为:
emissionprob_(MultinomialHMM模型中)或者再GaussianHMM模型中直接给定均值(means)和方差/协方差矩阵(covars).
'''
## MultinomialHMM案例
import numpy as np
...
n_states = len(states)
m_obs = len(obs)
start_probability = np.array([0.2, .5, .3])
